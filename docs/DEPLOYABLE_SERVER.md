...
- 认证接口：`POST /api/auth/login`、`POST /api/auth/logout`、`GET /api/auth/me`、`POST /api/auth/change-password`。
...
- 管理任务和统计接口：`GET /api/admin/jobs` 支持按用户、状态和任务类型筛选；管理员可通过 `POST /api/jobs/{job_id}/cancel` 取消任意普通生成任务，`DELETE /api/admin/jobs/{job_id}` 可删除 failed/canceled 任务记录，`GET /api/admin/stats` 返回用户、额度、任务状态、失败率、按模型聚合的用量、今日用量、用户消耗排行和高消耗任务。
- 管理用户接口：`GET /api/admin/users`、`POST /api/admin/users`、`PATCH /api/admin/users/{user_id}` 可创建用户并修改角色、状态、显示名和月额度；`POST /api/admin/users/{user_id}/reset-password` 可重置密码，`POST /api/admin/users/{user_id}/disable` 和 `POST /api/admin/users/{user_id}/enable` 可禁用或启用账号，`POST /api/admin/users/{user_id}/quota/add` 可手动增加额度。
...
普通生成任务取消支持当前服务进程内正在运行的任务：`POST /api/jobs/{job_id}/cancel` 会向正在运行的子进程发送终止信号，并在执行线程退出后把预扣额度退回；普通用户只能取消自己的任务，管理员可以取消任意普通生成任务。服务重启后已经失去进程句柄的历史任务只能查看状态和日志；如果需要跨进程、跨机器可靠取消，应继续引入 Redis/Celery/RQ 等任务队列。
